...
async function activate(context) {
    ...
    if (liveShare) {
        const session = await liveShare.session;
        if (session.id !== null) {
            vscode.window.showInformationMessage("You are in a Live Share session with id: " + session.id);
        }
        else {
            vscode.window.showInformationMessage("You are not in a Live Share session");
        }
